...
- **案例类型**：“显式多风险”、“语境边界”、“合规对照”只用于演示表达类型和识别难度，
  不代表审核结果；“语境边界”不等于“中风险”。
- **Finding Severity**：由系统按冻结 taxonomy/rule mapping 赋值，LLM 不能决定或修改。
- **综合风险等级**：取所有有效 Finding 的最高 severity。存在 High 则为高风险；否则存在
  Medium 则为中风险；无有效 Finding 则为低风险。

`0 Finding` / 低风险仅表示当前自动审核未形成已定义的有效风险项，不等于完整法律合规结论。
